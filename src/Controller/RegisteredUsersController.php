    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if (!$this->RegisteredUsers->exists(['username' => $registeredUser->username])&& !$this->RegisteredUsers->exists(['email' => $registeredUser->email])) 
            {
                ...
                $this->Auth->login();//login after registering
                //$authUser = $this->RegisteredUsers->get($result->id)->toArray();//
                //$this->Auth->setUser($authUser); //line for logging in after registering
