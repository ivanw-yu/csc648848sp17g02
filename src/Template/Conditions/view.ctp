...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('Edit Condition'), ['action' => 'edit', $condition->condition_name]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Condition'), ['action' => 'delete', $condition->condition_name], ['confirm' => __('Are you sure you want to delete # {0}?', $condition->condition_name)]) ?> </li>
...
<div class="conditions view large-9 medium-8 columns content">
    <h3><?= h($condition->condition_name) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Condition Name') ?></th>
            <td><?= h($condition->condition_name) ?></td>
        </tr>
    </table>
    <div class="related">
        <h4><?= __('Related Listings') ?></h4>
        <?php if (!empty($condition->listings)): ?>
        <table cellpadding="0" cellspacing="0">
            ...
            <?php foreach ($condition->listings as $listings): ?>
            <tr>
